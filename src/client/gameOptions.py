# Johns Hopkins University - Whiting School of Engineering
# Engineering for Professionals
# Spring 2019 - Foundations of Software Engineering
# TJ^3 Project Group
#
# Button Information is located in settings
# This file is for button functionality methods

from popDialog import *

# MAKE_SUGGESTION
def makeSuggestion(suspects, weapons, rooms):
    popDialog.popDialog(suspects, weapons, rooms)
    # PopUp
    #print(join_selection(suspects, True))
    #suspect = suspects[int(input('Enter suspect number:'))]
    #print(join_selection(rooms, False))
    #location = rooms[int(input('Enter location number :'))]
    #print(join_selection(weapons, False))
    #weapon = weapons[int(input('Enter weapon number:'))]
    #sio.emit('suggest', {"suspect":suspect, "location":location, "weapon":weapon}) #ex. 'Mrs. White', 'Study', 'Knife'

# MAKE_ACCUSATION
def makeAccusation(suspects, weapons, rooms):
    popDialog.popDialog(suspects, weapons, rooms)
    #print(join_selection(suspects, True))
    #suspect = suspects[int(input('Enter suspect number:'))]
    #print(join_selection(rooms, False))
    #location = rooms[int(input('Enter location number :'))]
    #print(join_selection(weapons, False))
    #weapon = weapons[int(input('Enter weapon number:'))]
    #sio.emit('accuse', {"suspect":suspect, "location":location, "weapon":weapon}) #ex. 'Mrs. White', 'Study', 'Knife'

# TO_STUDY
def passageToStudy():
    pass

# TO_KITCHEN
def passageToKitchen():
    pass

# TO_CONSERVATORY
def passageToConservatory():
    pass

# TO_LOUNGE
def passageToLounge():
    pass
